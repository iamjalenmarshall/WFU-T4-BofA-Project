...
DATA_DIR = _SCRIPT_DIR / "per_bank_nn"
LEVELS_DIR = _SCRIPT_DIR / "per_bank_nn_levels"
QOQ_RAW_DIR = _SCRIPT_DIR / "per_bank_qoq"
FEATURES_RAW_DIR = _SCRIPT_DIR / "per_bank_features"
ISO_DIR = _SCRIPT_DIR / "iso_output"
FORECAST_DIR = _SCRIPT_DIR / "forecasts_1"
...
def main():
    ...
    with tab_all:
        ...
        st.info(
            "**What is the unusualness score?** A 0–100 percentile rank combining three models: "
            "(1) a **QoQ neural network** autoencoder that learns normal quarter-over-quarter change patterns "
            "and flags quarters with high reconstruction error, ensembled with a Local Outlier Factor (LOF) detector; "
            "(2) a **Levels neural network** autoencoder trained on raw balance-sheet levels; and "
            "(3) an **Isolation Forest** on absolute values. "
            "A quarter is \"flagged\" when any model's score crosses its anomaly threshold. "
            "\"High confidence\" = flagged by **2 or more** of the 3 models.",
            icon="ℹ️",
        )
        ...
        for i, bank in enumerate(bank_names):
            ...
            with metric_cols[i]:
                st.metric(bank, f"{n_any} flagged", f"{n_high} high-confidence",
                          help="High confidence = flagged by 2 or more of the 3 detection models (QoQ NN, Levels NN, Isolation Forest).")

        st.divider()

        # Focus-bank area chart with other banks as context
        st.subheader(f"{selected_bank} vs. All Banks")
        st.caption(
            "The shaded area shows the selected bank's unusualness score over time. "
            "Grey lines show other banks for context. "
            "The dashed red line marks the threshold above which quarters are typically flagged."
        )

        # Build per-bank percentile scores from QoQ ensemble_score
        _all_bank_scores = {}
        for bname in bank_names:
            if bname in qoq_banks:
                bdf = _filter_years(_ensure_score_cols(qoq_banks[bname].copy()))
                sc = _score_col(bdf) or 'ensemble_score'
                if sc in bdf.columns and 'quarter_date' in bdf.columns:
                    bdf = bdf.sort_values('quarter_date').copy()
                    bdf['_pct'] = bdf[sc].rank(pct=True) * 100
                    _all_bank_scores[bname] = bdf

        if _all_bank_scores:
            fig_area = go.Figure()

            # Other banks — thin grey lines
            for bname, bdf in _all_bank_scores.items():
                if bname == selected_bank:
                    continue
                fig_area.add_trace(go.Scatter(
                    x=bdf['quarter_date'], y=bdf['_pct'],
                    mode='lines', name='Other banks',
                    line=dict(color='rgba(150,150,150,0.25)', width=1),
                    legendgroup='other',
                    showlegend=(bname == [b for b in _all_bank_scores if b != selected_bank][0]),
                    hoverinfo='skip',
                ))

            # Selected bank — filled area + solid line
            if selected_bank in _all_bank_scores:
                sdf = _all_bank_scores[selected_bank]
                fig_area.add_trace(go.Scatter(
                    x=sdf['quarter_date'], y=sdf['_pct'],
                    mode='lines', name=selected_bank,
                    line=dict(color=NORMAL_BLUE, width=2.5),
                    fill='tozeroy',
                    fillcolor='rgba(69,123,157,0.20)',
                    hovertemplate='%{customdata[0]}<br>Score: %{y:.0f}<extra></extra>',
                    customdata=sdf[['quarter']].values,
                ))
            # Threshold line at ~90th percentile
            fig_area.add_hline(
                y=90, line_dash='dash', line_color='rgba(230,57,70,0.45)',
                annotation_text='Typical flag threshold',
                annotation_position='top left',
                annotation_font_color='rgba(230,57,70,0.7)',
            )

            fig_area.update_layout(**plotly_theme_layout())
            fig_area.update_layout(
                yaxis_title="Unusualness score (0 = normal, 100 = most unusual)",
                height=420,
                legend=dict(orientation='h', yanchor='bottom', y=1.02),
            )
            fig_area.update_xaxes(showgrid=True, gridcolor='rgba(128,128,128,0.2)')
            fig_area.update_yaxes(showgrid=True, gridcolor='rgba(128,128,128,0.2)', range=[0, 105])
            st.plotly_chart(fig_area, use_container_width=True)
        else:
            st.info("No score data available for the selected year range.")
    ...
    with tab_bank:
        ...
        st.caption(
            "The unusualness score ranks each quarter from 0 (completely normal) to 100 "
            "(the most unusual ever observed). It combines signals from multiple detection "
            "models that analyze a bank's quarterly financial filings."
        )
        ...
        # --- Flagged quarters bar chart (replaces multi-line percentile chart) ---
        st.subheader("Flagged Quarters Over Time")
        st.caption(
            "Each bar shows how many of our three detection models flagged that quarter "
            "as unusual. **Taller, redder bars = stronger signal** that something noteworthy "
            "happened in that quarter."
        )

        if len(bank_conv) > 0:
            bc_chart = bank_conv.sort_values('quarter_date').copy()
            # Color map: 0=transparent, 1=gold, 2=orange, 3=red
            color_map = {0: 'rgba(180,180,180,0.15)', 1: '#E9C46A', 2: '#F4842A', 3: ANOMALY_RED}
            ...
            fig_bars.add_trace(go.Bar(
                ...
                hovertemplate='%{customdata[0]}<br>%{y} of 3 models flagged<extra></extra>',
            ...
            fig_bars.update_yaxes(
                ...
                range=[0, 3.5], dtick=1, tickvals=[0, 1, 2, 3],
            )
            st.plotly_chart(fig_bars, use_container_width=True)
        else:
            st.info("No data available for this bank.")
    ...
    # TAB 4: MODEL AGREEMENT (hidden — code removed)

    # =========================================================================
    # HEATMAP (hidden — uncomment to re-enable)
    # =========================================================================
    # with tab_heat:
    #     st.subheader("Anomaly Heatmap — All Banks Over Time")
    #     st.caption(
    #         "Each cell shows how many models flagged that bank's quarter. "
    #         "**Darker red = more models agree something unusual happened.** "
    #         "White cells = normal quarters."
    #     )
    #     if not conv_df.empty:
    #         heat_df = conv_df.pivot_table(
    #             index='Bank', columns='quarter_date', values='Models Flagged',
    #             aggfunc='max', fill_value=0,
    #         )
    #         heat_df = heat_df[sorted(heat_df.columns)]
    #         col_labels = [f"Q{((d.month - 1) // 3) + 1} {d.year}" for d in heat_df.columns]
    #         priority = ['Bank of America', 'Citibank', 'JPMorgan Chase Bank']
    #         others = [b for b in heat_df.index if b not in priority]
    #         others_sorted = sorted(others, key=lambda b: heat_df.loc[b].sum(), reverse=True)
    #         bank_order = [b for b in priority if b in heat_df.index] + others_sorted
    #         heat_df = heat_df.reindex(bank_order)
    #         fig_hm = go.Figure(data=go.Heatmap(
    #             z=heat_df.values, x=col_labels, y=heat_df.index,
    #             colorscale=[[0.0,'rgba(255,255,255,0.05)'],[0.33,'#FFEAA7'],[0.66,'#F4842A'],[1.0,ANOMALY_RED]],
    #             zmin=0, zmax=3,
    #             colorbar=dict(title="Models<br>Flagged", tickvals=[0,1,2,3], ticktext=["0","1","2","3"]),
    #             hovertemplate='%{y}<br>%{x}<br>Models flagged: %{z}<extra></extra>',
    #         ))
    #         fig_hm.update_layout(**plotly_theme_layout())
    #         fig_hm.update_layout(
    #             height=max(300, len(bank_order) * 80),
    #             xaxis=dict(showgrid=False, dtick=4, tickangle=-45),
